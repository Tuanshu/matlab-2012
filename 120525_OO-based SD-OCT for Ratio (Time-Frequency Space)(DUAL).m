...
cd('D:\120524\GLA_OPD1FOC1\inter\');
...
Filter_inter=0;
cd('D:\120524\SAM3_OPD1FOC1\inter\');
for j=0:99
...
Spectrum_Ratio=Spectrum_Sample./Spectrum_Reference;

plot(Frequency,Spectrum_Ratio);

Spectrum_Ratio=Spectrum_Ratio./Gauss_Window;

Spatial_Range=10:0.1:30;

for j=1:length(Spatial_Range)
    Spectrum_Ratio(N_f+1:N_t)=0;
    Signal_Ratio=fft(Spectrum_Ratio).*gaussmf(Position_micron,[Gauss_Window Spatial_Range(j)]);  
    Signal_Ratio(round(length(Signal_Ratio)/2)+1:end)=0;
    Signal_Ratio_Part=(ifft(Signal_Ratio));
    Time_Spectrum_Ratio(:,j)=2*Signal_Ratio_Part(1:N_f);
    
end
imagesc(abs(Time_Spectrum_Ratio),'xdata',Spatial_Range,'ydata',Frequency);
...
for j=1:length(Spatial_Range)
    ...
    Signal_Ratio(round(length(Signal_Ratio)/2)+1:end)=0;
    Signal_Ratio_Part=(ifft(Signal_Ratio));
    Time_Spectrum_Ratio_2(:,j)=2*Signal_Ratio_Part(1:N_f);
    
end
imagesc(abs(Time_Spectrum_Ratio_2),'xdata',Spatial_Range,'ydata',Frequency);

%% Dual

Time_Spectrum_Ratio_Dual=(Time_Spectrum_Ratio.*Time_Spectrum_Ratio_2).^0.5;


Frequency_max=6.5E14;
Frequency_min=4E14;
Frequency_max_index=find(Frequency>Frequency_max,1,'first');
Frequency_min_index=find(Frequency>Frequency_min,1,'first');
imagesc(10*log(abs(Time_Spectrum_Ratio(Frequency_min_index:Frequency_max_index,:))),'xdata',Spatial_Range,'ydata',Frequency(Frequency_min_index:Frequency_max_index));
imagesc(10*log(abs(Time_Spectrum_Ratio_2(Frequency_min_index:Frequency_max_index,:))),'xdata',Spatial_Range,'ydata',Frequency(Frequency_min_index:Frequency_max_index));
imagesc(10*log(abs(Time_Spectrum_Ratio_Dual(Frequency_min_index:Frequency_max_index,:))),'xdata',Spatial_Range,'ydata',Frequency(Frequency_min_index:Frequency_max_index));
