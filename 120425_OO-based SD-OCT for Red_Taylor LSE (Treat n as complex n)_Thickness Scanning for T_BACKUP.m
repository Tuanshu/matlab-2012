...
if index_calculation==1   
    for p=1:length(Parameter_1)
        for q=1:length(Parameter_2)
            ...
            Aexp=Spectrum_Devided(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index).*exp(-i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now));   %注意! -1*n!;
            ...
            while (j<Number_of_Loop)
                if mod(j,Number_of_Loop_Checking)==0
                    CONT_left=1;
                    CONT_right=1;
                ...
                delta_n = @ (n,n1_Considered,n2_Considered,r_BK7_Considered,Aexp) (Aexp-((1./r_BK7_Considered) .* ((n1_Considered-n)./(n1_Considered+n)+(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ ...
                          ...
                          ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)))))./((1./r_BK7_Considered) .* (-2*n1_Considered./(n1_Considered+n).^2 + (exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ...
                          ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ...
                ...
                n=n+0.1*delta_n(n,n1_Considered,n2_Considered,r_BK7_Considered,Aexp);
                j=j+1;
            end
...
plot(Wavelength_micron_Considered,real(n));
