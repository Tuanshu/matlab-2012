...
Ratio2=1;
Number_of_Loop=5;
Thickness=([-0.3:0.1:0.3]+2.7).*1E-6;
...
Wavelength_Considered_Min=450;          %nm
...
dA_dn = @ (n,k) -1./r_BK7 .* 2*n1./(n1+(n+i*k)).^2 + (exp(i*4*pi./Wavelength*(n+i*k)*d) ./ (1-(((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* exp(i*4*pi./Wavelength*(n+i*k)*d))) .* ((2*n1)/(n1+(n+i*k))) .* ((2*(n+i*k))./(n1+(n+i*k))) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* (-1./(n1+(n+i*k)) + n1./ (n1+(n+i*k)) ./ (n+i*k) + 2*n1 ./ ((n+i*k)+n2) ./ ((n+i*k)-n2) +i*4*pi*d/Wavelength + (exp(i*4*pi./Wavelength*(n+i*k)*d) ./ (1-(((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* exp(i*4*pi./Wavelength*(n+i*k)*d))) .* (((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .*(2*n1 ./ ((n+i*k)+n1) ./ ((n+i*k)-n1) + 2*n2 ./ ((n+i*k)+n2) ./ ((n+i*k)-n2) + i*4*pi*d./Wavelength));
dA_dk = @ (n,k) -i./r_BK7 .* 2*n1./(n1+(n+i*k)).^2 + (exp(i*4*pi./Wavelength*(n+i*k)*d) ./ (1-(((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* exp(i*4*pi./Wavelength*(n+i*k)*d))) .* ((2*n1)/(n1+(n+i*k))) .* ((2*(n+i*k))./(n1+(n+i*k))) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* (-1./(n1+(n+i*k)) + n1./ (n1+(n+i*k)) ./ (n+i*k) + 2*n1 ./ ((n+i*k)+n2) ./ ((n+i*k)-n2) +i*4*pi*d/Wavelength + (exp(i*4*pi./Wavelength*(n+i*k)*d) ./ (1-(((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .* exp(i*4*pi./Wavelength*(n+i*k)*d))) .* (((n+i*k)-n1)./((n+i*k)+n1)) .* (((n+i*k)-n2)./((n+i*k)+n2)) .*(2*n1 ./ ((n+i*k)+n1) ./ ((n+i*k)-n1) + 2*n2 ./ ((n+i*k)+n2) ./ ((n+i*k)-n2) + i*4*pi*d./Wavelength));
dAr_dn = @ (n,k);
dAr_dk = @ (n,k);
dAi_dn = @ (n,k);
dAi_dk = @ (n,k);
Ar = @ (n,k);
Ai = @ (n,k);
D = @ (n,k) dAr_dn(n,k).*dAi_dk(n,k)-dAr_dk(n,k).*dAi_dn(n,k);

delta_n = @ (n,k,Aexpr,Aexpi)  ((Aexpr-Ar(n,k,Aexpr)).*dAi_dk(n,k)-dAr_dk(n,k).*(Aexpi-Ai(n,k,Aexpi)))./D(n,k);
delta_k = @ (n,k,Aexpr,Aexpi)  (dAr_dn(n,k).*(Aexpi-Ai(n,k,Aexpi))-(Aexpr-Ar(n,k,Aexpr)).*dAi_dn(n,k))./D(n,k);
...
if index_calculation==1   
    ...
    Spectrum_Devided_Considered=Spectrum_Devided(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index);
    ...
    delta_k = @ (n,k,Aexpr,Aexpi)  (dAr_dn(n,k).*(Aexpi-Ai(n,k,Aexpi))-(Aexpr-Ar(n,k,Aexpr)).*dAi_dn(n,k))./D(n,k);
                ...
                Spectrum_Upper_Temp=((r1)./r_BK7(index_now)).*(d0.^2);                       %神說是cosine
                Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))./(1-r1_r.*(d.^2).*r2).*(d0.^2);                         %神說是cosine                
                ...
                t_sub=2.*n2./(n2+n1(index_now));
                
                T_abs=abs(t_sub.*t1.*t2.*(d)./(1-r2.*r1_r.*(d).^2)).^2;
                
                Merit=(((Spectrum_Temp)-(Ratio_Now*Spectrum_Devided(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
