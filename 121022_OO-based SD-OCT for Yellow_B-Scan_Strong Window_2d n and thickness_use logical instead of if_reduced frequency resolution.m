...
Ratio_Lower2Upper=1.25;%+[-0.2:0.02:0.2];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;

%%
DC_cutoff=55;   %works for all lateral position
Least_Separation=3;
Window_Size=2.8;  %use for the left side of 1st interface and right side of second interface
...
clear Spectrum_Sample_Frequency
...
if (Last_With_Second_Index>First_No_Second_Index)
    ...
    if xor(abs(Max_index(First_No_Second_Index)-Max_index(Last_With_Second_Index))>abs(Max_index(First_No_Second_Index)-Max_index_Second(Last_With_Second_Index)),mean(If_Second_Lower(1:Last_With_Second_Index))>0.5)
...
plot(Position_micron,Signal_Sample_1(1:5),Position_micron,Signal_Sample_2(1:5));
...
Signal_Reference=Signal_Sample_1(:,array_number_ref);
Spectrum_Reference=Spectrum_Sample_1(:,array_number_ref);
...
plot((1:length(Thickness_Profile))*100,Thickness_Profile);
...
Cexp=Cexp-repmat(Cexp(Center_Wavelength_micron_Index,:),size(Cexp,1),1);
...
C = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now) unwrap(angle(1./r1(n).*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(1i*4*pi.*Frequency_Considered.*n.*Thickness_Now/c)))),[],1);
...
Thickness_Now(1:length(Frequency_Considered),1:Active_Array_Size)=Thickness;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        n(isnan(n))=n_should;
                        n(isinf(n))=n_should;
                        Thickness_Now(isnan(Thickness_Now))=Thickness;
                        Thickness_Now(isinf(Thickness_Now))=Thickness;
                        %if Current_Loop>70
                        %    n_Record(:,qqqq)=n;
...
plot(Wavelength_micron_Considered,mean(real(n_Best),2),Wavelength_micron_Considered,n1_Considered);
xlabel('Wavelength (micron)');
ylabel('n');
...
plot(Wavelength_micron_Considered,mean(imag(n_Best),2));
xlabel('Wavelength (micron)');
ylabel('k');
...
plot((1:length(Thickness_Best))*100,Thickness_Best);
xlabel('Lateral Position (micron)');
