...
data_Bscan=importdata('D:\120131\3mm 0.1.txt');
...
index_boundary_at1stscan=10000;
...
for j=1:length(lateral_position)
    ...
    elseif (abs(axial_position(index_profile_max) - profile(j-1)) < 1)
        profile(j)=axial_position(index_profile_max);
    else
    ...
    elseif (abs(axial_position(index_ref_max+index_boundary_current) - profile_ref(j-1)) < 1)
        profile_ref(j)=axial_position(index_ref_max+index_boundary_current);
    else
...
plot(lateral_position,profile_sub,lateral_position,profile_new);
plot(lateral_position,profile_new);
