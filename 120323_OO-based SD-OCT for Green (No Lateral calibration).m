...
Position_Error=0.07*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.2;
...
if index_calculation == 1
%% Checking
T_abs_check(1:N_f,1:length(Position_Error))=0;
for j=1:length(Position_Error)
    if index_calculation == 1
...
%%
plot(Wavelength_micron(Wavelength_micron<0.65),k_final(Wavelength_micron<0.65,1));

xlabel('Wavelength (micron)');
ylabel('k');

plot(Wavelength_micron(Wavelength_micron<0.65),T_abs_check(Wavelength_micron<0.65,1));
