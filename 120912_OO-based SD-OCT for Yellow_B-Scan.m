...
Sample_Path='C:\TuanShu\120905_YPR Bscan\';
Reference_Path='C:\TuanShu\120905_YPR Bscan\ref\';
Spectroscopy_Path='C:\TuanShu\';
...
Ratio_Upper2Reference=1+[-0.2:0.01:0.2];
...
Ratio_Lower2Upper=1+[-0.2:0.01:0.2];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;

%%

Current_separation=44;
DC_cutoff=35;

DC_cutoff_ref=40;

High_Frequency_cutoff_ref=55;
...
%% Data Loading
for array_number=1:1%length(array)

cd(Sample_Path);
Filter_inter=importdata(sprintf('D%i.txt',array(array_number)));      % Data_2: the glass data 1
...
Glass_inter=importdata(sprintf('D%i.txt',array(array_number_ref)));      % Data_2: the glass data 1
...
Signal_Sample=fft(Spectrum_Sample);  
...
Signal_Sample=Signal_Sample.*Gauss_Window_Profile;

[value index]=max(abs(Signal_Sample(Position_micron<Current_separation)));

Gauss_Window_C_2=Position_micron(index)+3;
...
Signal_Sample_1=Signal_Sample.*Gauss_Window_Profile_2;


Gauss_Window_Profile_3=1-gaussmf(Position_micron,[Gauss_Window_sig Gauss_Window_C_2]);
Gauss_Window_Profile_3(Position_micron<Gauss_Window_C_2)=0;

Signal_Sample=Signal_Sample.*Gauss_Window_Profile_3;


[value index]=max(abs(Signal_Sample));

Gauss_Window_C_4=Position_micron(index)+3;


Gauss_Window_Profile_4=1-gaussmf(Position_micron,[Gauss_Window_sig Gauss_Window_C_4]);
Gauss_Window_Profile_4(Position_micron>Gauss_Window_C_4)=0;

Signal_Sample_2=Signal_Sample.*Gauss_Window_Profile_4;


%plot(Position_micron,Signal_Sample_1,Position_micron,Signal_Sample_2);


Current_separation=Gauss_Window_C_2;


Gauss_Window_C_ref=DC_cutoff_ref;

Gauss_Window_C_2_ref=High_Frequency_cutoff_ref;
...
Signal_Reference=Signal_Reference.*Gauss_Window_Profile_ref.*Gauss_Window_Profile_2_ref;
...
%plot(Wavelength_micron_Considered,T_Best,Wavelength_micron_Considered,T_Considered);
plot(Wavelength_micron_Considered,mean(T_Best,2),Wavelength_micron_Considered,T_Considered);
xlabel('Wavelength (micron)');
ylabel('T');
