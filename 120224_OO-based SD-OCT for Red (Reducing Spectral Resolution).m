...
clear all
index_calculation=1;
index_initial_boundary_reference=4500;
index_initial_boundary_sample=4500;

array=1:999;

index_used_reference_plane=800;

index_edge_range_left=430;

index_edge_range_right=460;


lateral_index_calibration_start=200;
lateral_index_calibration_end=900;

lateral_index_reference_start=600;
lateral_index_reference_end=800;

lateral_index_sample_start=300;
lateral_index_sample_end=300;
    ...
    for q=1:length(n_should)
        ...
        for p=1:Number_of_Loop
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                Spectrum_Temp=Spectrum_Upper_Temp+Spectrum_Lower_Temp;
                Merit=(((Spectrum_Temp)-(Spectrum_New(index_now)/Spectrum_Calibration(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
