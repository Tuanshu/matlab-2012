...
Distance_Sample_Post=(R_Sample_Post*1000)+CCT*1000-(((R_Sample_Post*1000)^2)-(X_Grid.^2)-(Y_Grid.^2)).^0.5;
...
Distance_Sample_Post=real(Distance_Sample_Post);
...
for j=1:length(Wavelength_Micron)
    ...
    Inter=Inter+(Spectrum_Normalized(j))+Spectrum_Normalized(j)*(exp(i.*4.*pi./Wavelength_Micron(j).*(Distance_Sample-Distance_Reference))+exp(i.*4.*pi./Wavelength_Micron(j).*(Distance_Sample_Post-Distance_Reference)));
end
Inter=real(Inter);
