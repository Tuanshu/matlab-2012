...
Ratio_Lower2Upper=1.08;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.2;
...
Bexp=abs(Spectrum_Devided(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index));   %注意! -1*n!;
Cexp=angle(Spectrum_Devided(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index));   %注意! -1*n!;
...
det_array_2 = @ (Q1,Q2,Q3,Q4) Q1.*Q4-Q2.*Q3;
            ...
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        dn_real = det_array_2(delta_A,dA_dn_imag,delta_C,dC_dn_imag)./det_array_2(dA_dn_real,dA_dn_imag,dC_dn_real,dC_dn_imag);
                        dn_imag = det_array_2(dA_dn_real,delta_A,dC_dn_real,delta_C)./det_array_2(dA_dn_real,dA_dn_imag,dC_dn_real,dC_dn_imag);
                        ...
                        %Thickness_Now=Thickness_Now+0.1*mean(dthickness);
                        ...
                        if Current_Loop<80 && Current_Loop>1
                            n_Record(:,qqqq)=n;
...
%plot(Wavelength_micron_Considered,Spectrum_Sample_Upper_Temp_Best,Wavelength_micron_Considered,Spectrum_Sample_Lower_Temp_Best,Wavelength_micron_Considered,Spectrum_Sample_Considered);
%plot(Wavelength_micron_Considered,T_Best,Wavelength_micron_Considered,T_Considered);
plot(Wavelength_micron_Considered,Cexp,Wavelength_micron_Considered,C(n_Best,Thickness_Best,Ratio_Lower2Upper_Best,Ratio_Upper2Reference_Best));
