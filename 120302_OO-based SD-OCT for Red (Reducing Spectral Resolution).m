...
for j=1:(lateral_index_calibration_end-lateral_index_calibration_start+1)    
    ...
    [maxvalue maxindex_calibration]=max(abs(Signal));
...
for j=1:(lateral_index_calibration_end-lateral_index_calibration_start+1)    
    ...
    [maxvalue maxindex_sample_reference]=max(abs(Signal));
    

    
    Profile_Sample_Reference(j+lateral_index_calibration_start-1)=Position(maxindex_sample_reference);
    index_Profile_Sample_Reference(j+lateral_index_calibration_start-1)=maxindex_sample_reference;
...
A=polyfit(array(lateral_index_reference_start:lateral_index_reference_end),Profile_Calibration(lateral_index_reference_start:lateral_index_reference_end)-Profile_Sample_Reference(lateral_index_reference_start:lateral_index_reference_end),1);
...
Profile_Sample_Reference=Profile_Sample_Reference-Profile_Calibration+Profile_Calibration(lateral_index_reference_start)+Distance_fit;
...
Signal_Sample_Reference((round(length(Signal_Sample_Reference)/2)+1):end)=0;
    
Spectrum_Used_Reference=ifft(Signal_Sample_Reference);
Spectrum_Used_Reference=2*Spectrum_Used_Reference(1:N_f);
        
Spectrum_Additional=exp(i*4*pi.*Frequency/C.*(Profile_Calibration(lateral_index_reference_start)-Profile_Calibration(index_used_reference_plane)));    %總之都移到Profile_Reference(1)
...
Spectrum_Used_Reference=Spectrum_Used_Reference(1:N_f);
