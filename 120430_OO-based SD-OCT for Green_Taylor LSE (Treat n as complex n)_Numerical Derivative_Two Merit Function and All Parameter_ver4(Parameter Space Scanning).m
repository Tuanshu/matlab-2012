...
Number_of_Loop=100;
Thickness=2.68.*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6;
Load_Previous_Result=0;
Position_Upper_Interface=-19.44.*1E-6;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.4;
...
Filter_sam=0;
cd('D:\120409-10(GREEN)\filter_sam\');
for j=0:0
...
for j=1:length(Parameter)
    ...
    %d0_check=exp(i*2*pi.*Frequency/C.*(Distance_fit(j)-Thickness_Now));   %注意! -1*n!
...
dlmwrite('Thickness_Now.txt',Thickness_Now,'delimiter','\t','newline','pc');
