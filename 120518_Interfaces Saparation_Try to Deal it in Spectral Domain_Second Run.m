...
xlabel('OPD (micron)');
Data_1_New=Spectrum_for_FFT(:,1);

Data_2_New=Spectrum_for_FFT(:,1)-amp_best*Spectrum_for_FFT(:,2).*exp(i*4*pi./Wavelength_micron_for_FFT.*shift_best);

amp_New=1.5:0.01:1.8;

Range_Lower_New=21;     %micron
Range_Upper_New=25;     %micron
Range_Lower_index_New=find(Position_micron>Range_Lower_New,1,'first');
Range_Upper_index_New=find(Position_micron>Range_Upper_New,1,'first');

MIN_2=9999999999999999999999;
for q=1:length(amp_New)
    Signal_Diff=fft(Data_1_New)-fft(amp_New(q)*Data_2_New);
    if max(real(Signal_Diff(Range_Lower_index_New:Range_Upper_index_New)))<MIN_2
        MIN_2=max(real(Signal_Diff(Range_Lower_index_New:Range_Upper_index_New)));
        amp_best_New=amp_New(q);
    end
end

plot(Position_micron,fft(Data_1_New),Position_micron,fft(amp_best_New*Data_2_New));
plot(Position_micron,fft(Data_1_New)-fft(amp_best_New*Data_2_New));
DD=fft(Data_1_New)-fft(amp_best_New*Data_2_New);
plot(Position_micron(Range_Lower_index_New:Range_Upper_index_New),DD(Range_Lower_index_New:Range_Upper_index_New));

Data_1_New_New=(Data_1_New)-(amp_best_New*Data_2_New);

plot(Position_micron,fft(Data_2_New)/max(fft(Data_2_New)),Position_micron,fft(Data_1_New_New)./max(fft(Data_1_New_New)));