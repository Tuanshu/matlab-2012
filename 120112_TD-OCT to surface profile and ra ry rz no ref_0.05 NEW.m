...
lateral_step=3;    %micron
length_lateral=2250;

window_ref=10;

data_Bscan=importdata('D:\120111\0.05_1.txt');
...
if ref_sub==1
    ref_Bscan=importdata('D:\120111\3mm ref2.txt');  
    ref_Bscan=ref_Bscan(:,1:round(length_lateral/lateral_step*3)); 
    ...
    for j=1:size(ref_Bscan_env,2)/3
        index_left_ref=find(ref_Bscan_env(:,1+(3*(j-1)))>value_max_ref(1+(3*(j-1)))/2,1,'first');
        index_right_ref=find(ref_Bscan_env(:,1+(3*(j-1)))>value_max_ref(1+(3*(j-1)))/2,1,'last');
