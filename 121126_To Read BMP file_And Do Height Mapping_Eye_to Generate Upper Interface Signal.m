...
image_index=200:1200;
ROI=[200 750;150 800];      %up, down, left, right
...
for q=1:length(image_index)
    ...
    imwrite(Temp_Slides,sprintf('New_%i.png',image_index(q)),'Bitdepth',new_bit_depth);
    disp(q);
end

for rr=1201:2499
    Temp_Slides=Image_Stack_New_Normalized(1,:,:);
    Temp_Slides(:,:)=1;
    imwrite(Temp_Slides,sprintf('New_%i.png',rr),'Bitdepth',new_bit_depth);
    disp(rr);
end
