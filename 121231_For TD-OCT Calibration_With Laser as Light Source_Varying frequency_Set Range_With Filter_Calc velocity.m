...
for p=1:length(Frequency)
    ...
    Time=(Voltage-5)/max_voltage/2/Frequency;
    ...
    Velocity(size(Velocity,1)+1)=Velocity(size(Velocity,1));
    Velocity=smooth(Velocity,smooth_window);

    Acceleration=diff(Velocity)./diff(Time);
    Acceleration(size(Acceleration,1)+1)=Acceleration(size(Acceleration,1));
    
    Acceleration=smooth(Acceleration,smooth_window);
    disp(p);
end
...
ylabel('Position (micron)','FontSize',18);

Position_Center_Zaro=Position-Position(find(Voltage>5,1,'first'));


Time=Time-Time(find(Voltage>5,1,'first'));

plot(Time,Acceleration);
xlabel('Time (sec)','FontSize',18);
ylabel('Acceleration (micron/sec/sec)','FontSize',18);

plot(Time,Velocity);
xlabel('Time (sec)','FontSize',18);
ylabel('Velocity (micron/sec)','FontSize',18);

Calibration_Data=[Time Position_Center_Zaro];
dlmwrite('Calibration_Data.txt',Calibration_Data,'delimiter','\t','newline','pc');
