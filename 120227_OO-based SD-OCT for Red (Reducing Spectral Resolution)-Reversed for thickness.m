...
clear all
index_calculation=0;
index_initial_boundary_reference=1300;
index_initial_boundary_sample=1300;

array=1:999;

index_used_reference_plane=600;

index_edge_range_left=370;

index_edge_range_right=400;


lateral_index_calibration_start=100;
lateral_index_calibration_end=700;

lateral_index_reference_start=450;
lateral_index_reference_end=700;

lateral_index_sample_start=100;
lateral_index_sample_end=700;
...
cd('D:\120222\R2 thick with ref 3\');
...
for j=1:(lateral_index_calibration_end-lateral_index_calibration_start+1)
    ...
    Signal_Sample_Reference(1:index_curernt_boundary)=0;
    ...
    Signal_Calibration((index_curernt_boundary+1):end)=0;
...
for j=1:(lateral_index_sample_end-lateral_index_sample_start+1)
    ...
    Signal(1:pixel_1)=0;
    Signal(round(length(Signal)/2):end)=0;
    [maxvalue maxindex]=max(abs(Signal((index_curernt_boundary+1):end)));
    ...
    Signal_Sample(1:index_curernt_boundary)=0;
...
dlmwrite('T_abs_check.txt',T_abs_check,'delimiter','\t','newline','pc');
    ...
    imagesc(10*log10(abs(B_scan)/max(max(abs(B_scan)))),'ydata',Position(1:round(length(Signal_New)/4)));
end