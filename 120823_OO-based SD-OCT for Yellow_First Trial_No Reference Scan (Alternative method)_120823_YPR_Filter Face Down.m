...
Ratio_Upper2Reference=1;%1.2+[0:0.05:0.4];
...
Separation_Position=37.3;   %(micron)
Gauss_Window_sig=1;
Gauss_Window_C=10;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference)
                    ...
                    %Bexp=Spectrum_Devided_2(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index);   %注意! -1*n!;
                    ...
                    %B = @ (n,Thickness_Now,Ratio_Lower2Upper_Now) Ratio_Lower2Upper_Now./r1(n).*(t1(n).*t1_r(n).*r2(n).*exp(i*4*pi.*Frequency_Considered.*n.*Thickness_Now/C)./(1-r1_r(n).*r2(n).*exp(i*4*pi*Frequency_Considered.*n.*Thickness_Now/C)));                    
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        n=n+0.1*((Aexp-A(n,Ratio_Upper2Reference_Now,Position_Upper2Reference))./((A(n+dn,Ratio_Upper2Reference_Now,Position_Upper2Reference)-A(n,Ratio_Upper2Reference_Now,Position_Upper2Reference))/dn));
