...
Thickness_Water_Layer=550.*1E-6;  %mm
...
Spectrum_After_Water=Spectrum_New.*exp(i*4*pi.*Frequency_New/C.*n_Water.*(Thickness_Water_Layer));
...
[value_2 index_2]=max(Signal_After_Water);

Signal_After_Water_Record=Signal_After_Water./abs(max(Signal));
...
FWHM_PSF(p)=Position_micron(find(Signal_After_Water_Record/max(Signal_After_Water_Record)>0.5,1,'last'))-Position_micron(find(Signal_After_Water_Record/max(Signal_After_Water_Record)>0.5,1,'first'));

end

dlmwrite('FWHM.txt',FWHM,'delimiter','\t','newline','pc');
dlmwrite('FWHM_PSF.txt',FWHM_PSF,'delimiter','\t','newline','pc');
