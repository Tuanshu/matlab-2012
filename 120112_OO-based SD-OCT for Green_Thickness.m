...
Number_of_Loop=5;
Thickness=2.56.*1E-6;%+[-0.1:0.01:0.1].*1E-6;
Load_Previous_Result=0;
Position_Error=0;%-1.195175706296598*1E-6+0.19*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
DC_phase=0;
n_should=1.65;
delta_n=0.1;
...
Data=importdata('10.2_2.txt');
Data_Reference=importdata('11.6_2.txt');
...
Spectrum_Additional=exp(i*4*pi.*Frequency/C.*(Position_Error)).*exp(i.*DC_phase.*2*pi);
...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        d=exp(i*2*pi.*Frequency(index_now)/C.*(n_temp+i*k(index_now)).*Thickness(q));   %注意! -1*n!
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        d=exp(i*2*pi.*Frequency(index_now)/C.*(n(index_now)+i*k_temp).*Thickness(q));   %注意! -1*n!
        
        d_n=exp(i*2*pi.*Frequency(index_now)/C.*(n(index_now)).*Thickness(q));   %注意! -1*n!
        d_k=exp(i*2*pi.*Frequency(index_now)/C.*(i*k_temp).*Thickness(q));   %注意! -1*n!
        %d0=exp(i*2*pi.*Frequency(index_now)/C.*Thickness_0);   %注意! -1*n!
        %dref=exp(i*2*pi.*Frequency(index_now)/C.*(Thickness_0+Thickness));   %注意! -1*n!
        Spectrum_Upper_Temp=((r1)./r_BK7(index_now));                       %神說是cosine
