...
Sample_Path='D:\120806_Yellow PR\5 micron\';
Reference_Path='D:\120806_Yellow PR\5 micron\';
Spectroscopy_Path='D:\120806_Yellow PR\';
...
Number_of_Loop=100;
Thickness=([-0.2:0.01:0.2]+4.4).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6;
Load_Previous_Result=0;
Ratio_Lower2Upper=0.5:0.05:2;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.4;
...
Separation_Position=24.75;   %(micron)
Gauss_Window=1;
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Reference2Sample)
                for m=1:length(Ratio_Upper2Lower)
                    ...
                    %A = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now) (exp(i*4*pi.*Frequency_Considered/C.*(Ratio_Lower2Upper_Now))./r_BK7_Considered./Ratio_Reference2Sample_Now) .* (Ratio_Upper2Lower_Now.*(n1_Considered-n)./(n1_Considered+n)+(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)));
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        Current_Progress=100*((p-1)+((q-1)+((w-1)+(m-1)/length(Ratio_Upper2Lower))/length(Ratio_Reference2Sample))/length(Ratio_Lower2Upper))/length(Thickness);
...
%Spectrum_Upper_Temp_Best=(exp(i*4*pi.*Frequency_Considered/C.*(Ratio_Lower2Upper_Best))./r_BK7_Considered./Ratio_Reference2Sample_Best) .* (Ratio_Upper2Lower_Best.*(n1_Considered-n_Best)./(n1_Considered+n_Best));
%Spectrum_Lower_Temp_Best=(exp(i*4*pi.*Frequency_Considered/C.*(Ratio_Lower2Upper_Best))./r_BK7_Considered./Ratio_Reference2Sample_Best) .* ((exp(i*4*pi.*Frequency_Considered/C.*n_Best*Thickness_Best) ./ (1-((n_Best-n1_Considered)./(n_Best+n1_Considered)) .* ((n_Best-n2_Considered)./(n_Best+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n_Best*Thickness_Best))) .* ((2*n1_Considered)./(n1_Considered+n_Best)) .* ((2*n_Best)./(n1_Considered+n_Best)) .* ((n_Best-n2_Considered)./(n_Best+n2_Considered)));
