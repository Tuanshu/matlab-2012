...
FWHM_Frequency=(FWHM(p)*1E-9)*C/((Wavelength_Center*1E-9)^2);
Spectrum_New=gaussmf(Frequency_New,[FWHM_Frequency/2.355 Frequency_Center]);
...
Thickness_Water_Layer=[500:5:600].*1E-6;  %mm
...
Measured_Thickness=(OPD_Measured_micron-OPD_Fitted)/fitting(1);
OPD_Measured_micron_diff=diff(OPD_Measured_micron);
OPD_Measured_micron_diff(length(OPD_Measured_micron_diff)+1)=OPD_Measured_micron_diff(length(OPD_Measured_micron_diff));
Group_Delay_Dispersion(p)=std(OPD_Measured_micron_diff./(OPD_Measured_micron));
Merit(p)=std(Thickness_Water_Layer_micron./(OPD_Measured_micron))*mean(OPD_Measured_micron);
%Group_Thickness_Dispersion(p)=std(diff(OPD_Measured_micron))/mean(OPD_Measured_micron)/(5E-6)*550E-6;

Measured_Thickness_std(p)=std(Measured_Thickness);
