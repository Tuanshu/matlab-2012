...
Averaging=100;    %Number of frame averaging
Sample_Path='D:\Users\TuanShu\120524\SAM7_OPD1FOC1\';
Reference_Path='D:\Users\TuanShu\120524\GLA_OPD1FOC1\';
Spectroscopy_Path='D:\Users\TuanShu\120524\';
...
Number_of_Loop=75;
Thickness=(2.53).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6; In this file, it is the initial guess of thickness
Load_Previous_Result=0;
Ratio_Lower2Upper=1;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.2;
...
%end
Filter_inter=importdata('Filter_inter.txt');  
%Filter_inter=Filter_inter/Averaging;

%dlmwrite('Filter_inter.txt',Filter_inter,'delimiter','\t','newline','pc');
...
Filter_sam=importdata('Filter_sam.txt');  
%Filter_sam=Filter_sam/Averaging;

%dlmwrite('Filter_sam.txt',Filter_sam,'delimiter','\t','newline','pc');
...
Filter_ref=importdata('Filter_ref.txt');  
%dlmwrite('Filter_ref.txt',Filter_ref,'delimiter','\t','newline','pc');
...
Filter_bs=importdata('Filter_bs.txt');  
%dlmwrite('Filter_bs.txt',Filter_bs,'delimiter','\t','newline','pc');
...
%dlmwrite('Glass_inter.txt',Glass_inter,'delimiter','\t','newline','pc');

Glass_inter=importdata('Glass_inter.txt');  
...
%dlmwrite('Glass_sam.txt',Glass_sam,'delimiter','\t','newline','pc');

Glass_sam=importdata('Glass_sam.txt');  
...
%dlmwrite('Glass_ref.txt',Glass_ref,'delimiter','\t','newline','pc');

Glass_ref=importdata('Glass_ref.txt');  
...
%dlmwrite('Glass_bs.txt',Glass_bs,'delimiter','\t','newline','pc');

Glass_bs=importdata('Glass_bs.txt');  
...
Separation_Position=29.25;   %(micron)
...
plot(Wavelength_micron_Considered*1000,T_Best,Wavelength_micron_Considered*1000,T_Considered);
xlabel('Wavelength (nm)');
ylabel('T');
