...
Spectrum_All(:,jj)=Spectrum_New;
Signal_All(:,jj)=Signal;
end
Spectrum_New=Spectrum_All(:,1)./Spectrum_All(:,2);

%%
%Carrier_Function=Amp*cos(4*pi*Frequency*Thickness/C+Phi);

%A= @(Amp,Thickness,Phi) abs(Spectrum_New).*(Amp*exp(i*4*pi*Frequency_New*Thickness/C+Phi));
A= @(Amp,Thickness,Phi) real(Amp*exp(i*4*pi*Frequency_New*Thickness/C+Phi));
dAmp=0.0001;
dThickness=1E-12;
dPhi=0.0001;
...
while (Current_Iteration<Number_of_Iteration)
    ...
    Amp=(ita_Amp*sum(Spectral_Array_delta_Amp.*abs(Spectrum_All(:,1)))./sum(abs(Spectrum_All(:,1))))+Amp;
    Thickness=(ita_Thickness*sum(Spectral_Array_delta_Thickness.*abs(Spectrum_All(:,1)))./sum(abs(Spectrum_All(:,1))))+Thickness;
    Phi=(ita_Phi*sum(Spectral_Array_delta_Phi.*abs(Spectrum_All(:,1)))./sum(abs(Spectrum_All(:,1))))+Phi;
end
