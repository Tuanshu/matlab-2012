...
Position_Error=19.2*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.2;
...
cd('D:\120409-10(GREEN)\Filter_inter\');
...
Filter_inter=0;
cd('D:\120409-10(GREEN)\filter_inter 2\');
for j=0:0
...
Filter_sam=0;
cd('D:\120409-10(GREEN)\filter_sam 2\');
for j=0:0
...
Glass_inter=0;
cd('D:\120409-10(GREEN)\Glass_inter\');
for j=0:0
...
Glass_sam=0;
cd('D:\120409-10(GREEN)\Glass_sam\');
for j=0:0
...
Signal_Sample=fft(Spectrum_Sample);%.*gaussmf(Position_micron,[Gauss_Window 31]);  
...
Signal_Reference=fft(Spectrum_Reference);%.*gaussmf(Position_micron,[Gauss_Window 31]);  
...
if index_calculation==1   
    ...
    elseif strcmp(Scanning,'Ratio')
        ...
        n_o=(n_should_Now-delta_n):0.0001:(n_should_Now+delta_n);
        k_o=-0.1:0.0002:0.3;
        %k_o=k_o';
        ...
        for p=1:Number_of_Loop
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                Merit=(((Spectrum_Temp)-(Ratio_Now*Spectrum_Devided(index_now).*(d0.^2))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
...
plot(Wavelength_micron(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index),n_final(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,4));

xlabel('Wavelength (micron)');
ylabel('n');

%%
plot(Wavelength_micron(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index),k_final(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,4));

xlabel('Wavelength (micron)');
ylabel('k');
%%
plot(Wavelength_micron(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index),T_abs_check(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,4));
