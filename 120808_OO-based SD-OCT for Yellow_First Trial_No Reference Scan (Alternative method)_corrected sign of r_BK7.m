...
Sample_Path='D:\120816_YPR\';
Reference_Path='D:\120808_YPR2\glass\';
...
Ratio_Upper2Reference=1.27;
Position_Upper2Reference_Now=1.38;
Number_of_Loop=100;
Thickness=(4.4).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6;
Load_Previous_Result=0;
Ratio_Lower2Upper=0.9;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.4;
...
Separation_Position=76.2;   %(micron)
Gauss_Window_sig=1;
Gauss_Window_C=50;
...
Spectrum_Devided=Spectrum_Sample_2./Spectrum_Sample_1;
...
plot(Wavelength_micron,Spectrum_Sample_1,Wavelength_micron,Spectrum_Sample_2);
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference_Now)
                    ...
                    %A = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now,Position_Upper2Reference_Now_Now) (exp(i*4*pi.*Frequency_Considered/C.*(Ratio_Lower2Upper_Now))./r_BK7_Considered./Ratio_Upper2Reference_Now) .* (Position_Upper2Reference_Now_Now.*(n1_Considered-n)./(n1_Considered+n)+(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)));
                    ...
                    B = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now,Position_Upper2Reference_Now) exp(i*4*pi.*Frequency_Considered.*Position_Upper2Reference_Now/C).*Position_Upper2Reference_Now./r_BK7_Considered.*(r_1+Ratio_Lower2Upper_Now.*(t1.*t1_r.*r2.*exp(i*2.*Frequency_Considered.*n.*Thickness_Now/C)./(1-r1_r.*r2.*exp(i*2*Frequency_Considered.*n.*Thickness_Now/C))));
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        Current_Progress=100*((p-1)+((q-1)+((w-1)+(m-1)/length(Position_Upper2Reference_Now))/length(Ratio_Upper2Reference))/length(Ratio_Lower2Upper))/length(Thickness);
                        ...
                        if Merit < Merit_Best
                            ...
                            Position_Upper2Reference_Now_Best=Position_Upper2Reference_Now_Now;
...
%Spectrum_Upper_Temp_Best=(exp(i*4*pi.*Frequency_Considered/C.*(Ratio_Lower2Upper_Best))./r_BK7_Considered./Ratio_Upper2Reference_Best) .* (Position_Upper2Reference_Now_Best.*(n1_Considered-n_Best)./(n1_Considered+n_Best));
