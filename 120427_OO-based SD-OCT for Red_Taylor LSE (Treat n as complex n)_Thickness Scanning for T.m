...
if index_calculation==1   
    ...
    for p=1:length(Parameter_1)
        for q=1:length(Parameter_2)
            
            if strcmp(Scanning_1,'n_should')
                n_should_Now=n_should(p);
                if strcmp(Scanning_2,'Thickness')
                    Thickness_Now=Thickness(q);
                    Ratio_Reference2Sample_Now=Ratio_Reference2Sample;
                    Ratio_Upper2Lower_Now=Ratio_Upper2Lower;
                    Position_Upper_Interface_Now=Position_Upper_Interface;
                    
                elseif strcmp(Scanning_2,'Ratio_Reference2Sample')
                    Ratio_Reference2Sample_Now=Ratio_Reference2Sample(q);
                    Ratio_Upper2Lower_Now=Ratio_Upper2Lower;
            ...
            elseif strcmp(Scanning_1,'Position_Upper_Interface')
                Position_Upper_Interface_Now=Position_Upper_Interface(p);
                if strcmp(Scanning_2,'Thickness')
                    Thickness_Now=Thickness(q);
                    n_should_Now=n_should;
                    Ratio_Reference2Sample_Now=Ratio_Reference2Sample;
                ...
                elseif strcmp(Scanning_2,'Ratio_Reference2Sample')
                    ...
                    Thickness_Now=Thickness;
                    n_should_Now=n_should;
                elseif strcmp(Scanning_2,'n_should')
                    n_should_Now=n_should(q);
                    Ratio_Reference2Sample_Now=Ratio_Reference2Sample;
                    Ratio_Upper2Lower_Now=Ratio_Upper2Lower;
                    Thickness_Now=Thickness;
            ...
            elseif strcmp(Scanning_1,'Ratio_Reference2Sample')
                Ratio_Reference2Sample_Now=Ratio_Reference2Sample(p);
                if strcmp(Scanning_2,'Thickness')
                    Thickness_Now=Thickness(q);
                    n_should_Now=n_should;
                    Position_Upper_Interface_Now=Position_Upper_Interface;
            ...
            elseif strcmp(Scanning_1,'Ratio_Upper2Lower')
                Ratio_Upper2Lower_Now=Ratio_Upper2Lower(p);
                if strcmp(Scanning_2,'Thickness')
                    Thickness_Now=Thickness(q);
                    n_should_Now=n_should;
                    Position_Upper_Interface_Now=Position_Upper_Interface;
            ...
            delta_n = @ (n) (Aexp-((exp(i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now))./r_BK7_Considered./Ratio_Reference2Sample_Now) .* (Ratio_Upper2Lower.*(n1_Considered-n)./(n1_Considered+n)...
            +(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now)))...
            .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)))))./((exp(i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now))...
            ./r_BK7_Considered./Ratio_Reference2Sample_Now) .* (Ratio_Upper2Lower.*(-2*n1_Considered./(n1_Considered+n).^2) + (exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered))  ...
            .*((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n))  ...
            .*((n-n2_Considered)./(n+n2_Considered)) .* (-1./(n1_Considered+n) + n1_Considered./ (n1_Considered+n) ./ n + 2*n2_Considered ./ (n+n2_Considered) ...
            ./ (n-n2_Considered)+i*4*pi*Thickness_Now.*Frequency_Considered/C + (exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) ...
            .* ((n-n2_Considered)./(n+n2_Considered)).* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) ...
            .*(2*n1_Considered ./ (n+n1_Considered) ./ (n-n1_Considered) + 2*n2_Considered ./ (n+n2_Considered) ./ (n-n2_Considered) + i*4*pi*Thickness_Now.*Frequency_Considered/C))));
            ...
            while (Current_Loop<Number_of_Loop)
                ...
                %A= @ (n,n1_Considered,n2_Considered,r_BK7_Considered) ((1./r_BK7_Considered) .* ((n1_Considered-n)./(n1_Considered+n)+(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ ...
                %          (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .*...
                %          ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered))));
                %dA_dn= @ (n,n1_Considered,n2_Considered,r_BK7_Considered,Aexp) ((1./r_BK7_Considered) .* (-2*n1_Considered./(n1_Considered+n).^2 + (exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ...
                %          ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ...
                %          ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)) .* (-1./(n1_Considered+n) + n1_Considered./ (n1_Considered+n) ./ n + 2*n2_Considered ./ (n+n2_Considered) ./ (n-n2_Considered)...
                %          +i*4*pi*Thickness_Now.*Frequency_Considered/C + (exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered))...
                %          .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .*(2*n1_Considered ./ (n+n1_Considered) ./ ...
                %          (n-n1_Considered) + 2*n2_Considered ./ (n+n2_Considered) ./ (n-n2_Considered) + i*4*pi*Thickness_Now.*Frequency_Considered/C))));
                n=n+0.1*delta_n(n);
                Current_Loop=Current_Loop+1;
            end
            T_abs=abs((2.*n2_Considered./(n2_Considered+n1_Considered)).*(2*n1_Considered./(n+n1_Considered)).*(2*n./(n+n2_Considered)).*exp(i*2*pi.*Frequency_Considered/C.*n.*Thickness_Now)./(1-((n-n1_Considered)./(n+n1_Considered)).*((n-n2_Considered)./(n+n2_Considered)).*exp(i*4*pi.*Frequency_Considered/C.*n.*Thickness_Now))).^2;
            Merit=sum((T_Considered-T_abs).^2);
            if Merit < Merit_Best
                Merit_Best=Merit;
                Parameter_1_Best=Parameter_1(p);
                Parameter_2_Best=Parameter_2(q);
                n_Best=n;
                T_Best=T_abs;
            end
...
for j=1:length(Parameter)
    ...
    %T_abs=abs((2.*n2./(n2+n1)).*(2*n1./(n+n1)).*(2*n./(n+n2)).*exp(i*2*pi.*Frequency/C.*n.*Thickness_Now)./(1-((n-n1)./(n+n1)).*((n-n2)./(n+n2)).*exp(i*4*pi.*Frequency/C.*n.*Thickness_Now))).^2;
