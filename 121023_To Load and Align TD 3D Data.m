...
cd('D:\Users\TuanShu\121022\3D\sample04_1');

file_name='sample04';
file_index=[1:200];


upper_range_of_1st_frame_1st_line=800;

lower_range_of_1st_frame_1st_line=850;
...
for p=1:length(file_index)
    Frame=importdata(sprintf('%s_%i.txt',file_name,file_index(p)));
    for q=1:size(Frame,2)
        if q==1
            [max_value max_index]=max(Frame(upper_range_of_1st_line:lower_range_of_1st_line,q));
            real_max_index=(max_index+upper_range_of_1st_line-1);
        else
            [max_value max_index]=max(Frame(upper_range:lower_range,q));
            real_max_index=(max_index+upper_range-1);
        end
        Frame(:,q)=circshift(Frame(:,q),select_height-real_max_index);
    ...
    dlmwrite(new_file_name,Frame,'delimiter','\t','newline','pc');
    fprintf('%d\n',p);
end
 imagesc(Frame,'ydata',(1:size(Frame,1))*0.06,'xdata',(1:size(Frame,2))*5); %3V