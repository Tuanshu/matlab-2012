...
data_Bscan=importdata('D:\120131\3mm 0.1 2.txt');
...
for j=1:length(lateral_position)
    ...
    elseif (abs(axial_position(index_profile_max) - profile(j-1)) < 2)
        profile(j)=axial_position(index_profile_max);
    else
    ...
    elseif (abs(axial_position(index_ref_max+index_boundary_current) - profile_ref(j-1)) < 2)
        profile_ref(j)=axial_position(index_ref_max+index_boundary_current);
    else
