...
Data=importdata(sprintf('D%i.txt',10));

Data_Reference=importdata(sprintf('D%i.txt',390));


cd('D:\120109_Red Color Filter 2_ref\');

Data_ref=importdata(sprintf('D%i.txt',10));

Data_Reference_ref=importdata(sprintf('D%i.txt',390));

Wavelength=Data(:,1);           %nm
Spectrum_Old=Data(:,2);
Spectrum_Reference_Old=Data_Reference(:,2);

Spectrum_ref_Old=Data_ref(:,2);
Spectrum_Reference_ref_Old=Data_Reference_ref(:,2);
...
Spectrum_ref_Frequency=(Spectrum_ref_Old.*((Wavelength*1E-9).^2)/C);        %/max(Spectrum_Old.*((Wavelength*1E-9).^2)/C);  N more normalization
Spectrum_Reference_ref_Frequency=(Spectrum_Reference_ref_Old.*((Wavelength*1E-9).^2)/C);    %/max(Spectrum_Reference_Old.*((Wavelength*1E-9).^2)/C);
...
Spectrum_ref=interp1(Frequency_Old,Spectrum_ref_Frequency,Frequency);
Spectrum_Reference_ref=interp1(Frequency_Old,Spectrum_Reference_ref_Frequency,Frequency);
...
Spectrum_ref(isnan(Spectrum_ref))=0;
Spectrum_ref(Frequency<Min_Frequency)=0;

Spectrum_Reference_ref(isnan(Spectrum_Reference_ref))=0;
Spectrum_Reference_ref(Frequency<Min_Frequency)=0;
...
Spectrum_ref((N_f+1):N_t)=0;
Spectrum_Reference_ref((N_f+1):N_t)=0;
...
Signal_ref=fft(Spectrum_ref);
Signal_Reference_ref=fft(Spectrum_Reference_ref);
...
pixel_1=520;               % the end of 1
pixel_2=1600;%2000               % the end of 2
pixel_3=900;               %for saperation
...
Signal_Reference_ref(1:pixel_1)=0;
Signal_Reference_ref((pixel_2+1):end)=0;

[max_value_Signal_ref max_index_Signal_ref]=max(abs(Signal_ref));
[max_value_Signal_Reference_ref max_index_Signal_Reference_ref]=max(abs(Signal_Reference_ref));

Position_Error=(Position_micron(max_index_Signal_ref)-Position_micron(max_index_Signal_Reference_ref)); %the ref position of signal and Reference
%Discussion: instead of using (exp) data/Reference, one should use
%data/data_ref/(Reference/Reference_ref)=data/Reference/(data_ref/Reference
%_ref),(data_ref/Reference_ref)=Spectrum_Additional(Position_Error)
...
%Spectrum_Reference_New=Spectrum_Reference_New.*exp(i*4*pi.*Frequency/C.*(Position_Error)); 
...
plot(Wavelength_micron(Wavelength_micron<0.7),n_final(Wavelength_micron<0.7));
plot(Wavelength_micron(Wavelength_micron<0.7),k_final(Wavelength_micron<0.7,:));

plot(Wavelength_micron(Wavelength_micron<0.7),Spectrum_Upper_Check(Wavelength_micron<0.7,:));
