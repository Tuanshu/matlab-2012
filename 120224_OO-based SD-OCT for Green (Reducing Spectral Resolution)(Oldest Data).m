...
clear all
index_calculation=1;
index_initial_boundary_reference=3500;
index_initial_boundary_sample=3500;

array=1:999;

index_used_reference_plane=760;

index_edge_range_left=625;

index_edge_range_right=650;


lateral_index_calibration_start=50;
lateral_index_calibration_end=250;

lateral_index_reference_start=50;
lateral_index_reference_end=50;

lateral_index_sample_start=150;
lateral_index_sample_end=250;
...
%    if abs((maxindex_sample_reference+maxindex_calibration+index_curernt_boundary)/2-index_curernt_boundary) < 100
%        index_curernt_boundary=round((maxindex_sample_reference+maxindex_calibration+index_curernt_boundary)/2);
%    end 
...
Spectrum_Additional=1;%exp(i*4*pi.*Frequency/C.*(Profile_Calibration_Shifted(lateral_index_reference_start)-Profile_Calibration_Shifted(index_used_reference_plane)));    %總之都移到Profile_Reference(1)
Spectrum_Additional_Based=1;%exp(i*4*pi.*Frequency/C.*(Distance_fit(index_used_reference_plane)));      
...
for j=1:(lateral_index_sample_end-lateral_index_sample_start+1)
    ...
    Spectrum_Additional=1;%exp(i*4*pi.*Frequency/C.*(Profile_Calibration_Shifted(lateral_index_reference_start)-Profile_Calibration_Shifted(j+lateral_index_sample_start-1)));    %總之都移到Profile_Reference(1)
    Spectrum_Additional_1;%Based=exp(i*4*pi.*Frequency/C.*(Distance_fit(j+lateral_index_sample_start-1)));    
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                if range_specified == 1
                    ...
                    elseif (index_now > Frequency_Center_Index) || (index_now < (Frequency_Center_Index -1))
                        range_upper=min(index_old+round(delta_n_number/100),length(Merit));
                        range_lower=max(index_old-round(delta_n_number/100),1);
                    end
...
plot(Wavelength_micron(Wavelength_micron<0.6),n_final(Wavelength_micron<0.6,:));
plot(Wavelength_micron(Wavelength_micron<0.6),k_final(Wavelength_micron<0.6,:));

plot(Wavelength_micron(Wavelength_micron<0.6),T_abs_check(Wavelength_micron<0.6,:));
