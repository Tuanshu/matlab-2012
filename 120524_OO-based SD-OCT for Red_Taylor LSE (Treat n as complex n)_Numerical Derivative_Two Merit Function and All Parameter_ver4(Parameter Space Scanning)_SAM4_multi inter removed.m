...
Ratio_Reference2Sample=1.1;
Ratio_Upper2Lower=0.8;
Number_of_Loop=100;
Thickness=([-0.2:0.02:0.2]+2.53).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6;
Load_Previous_Result=0;
Position_Upper_Interface=([-0.4:0.04:0.4]+13.1).*1E-6;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.4;
...
cd('D:\120523\Dispersion Compensated\GLA3_OPD1FOC1\inter\');
...
Filter_inter=0;
cd('D:\120523\Dispersion Compensated\SAM4_OPD3FOC1\inter\');
for j=0:99
...
Filter_sam=0;
cd('D:\120523\Dispersion Compensated\SAM4_OPD3FOC1\sam\');
for j=0:99
...
Filter_ref=0;
cd('D:\120523\Dispersion Compensated\SAM4_OPD3FOC1\ref\');
for j=0:99
...
Filter_bs=0;
cd('D:\120523\Dispersion Compensated\GLA3_OPD1FOC1\bs\');
for j=0:99
...
Glass_sam=0;
cd('D:\120523\Dispersion Compensated\GLA3_OPD1FOC1\sam\');
for j=0:99
...
Glass_ref=0;
cd('D:\120523\Dispersion Compensated\GLA3_OPD1FOC1\ref\');
for j=0:99
        ...
        for q=1:length(Position_Upper_Interface)
            for w=1:length(Ratio_Reference2Sample)
                for m=1:length(Ratio_Upper2Lower)
                    ...
                    A = @ (n,Thickness_Now,Position_Upper_Interface_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now) (exp(i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now))./r_BK7_Considered./Ratio_Reference2Sample_Now) .* (Ratio_Upper2Lower_Now.*(n1_Considered-n)./(n1_Considered+n)+(exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now)) .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)));
...
dlmwrite('Spectrum_Upper_Temp_Best_real.txt',real(Spectrum_Upper_Temp_Best),'delimiter','\t','newline','pc');
dlmwrite('Spectrum_Lower_Temp_Best_real.txt',real(Spectrum_Lower_Temp_Best),'delimiter','\t','newline','pc');

dlmwrite('Spectrum_Upper_Temp_Best_imag.txt',imag(Spectrum_Upper_Temp_Best),'delimiter','\t','newline','pc');
dlmwrite('Spectrum_Lower_Temp_Best_imag.txt',imag(Spectrum_Lower_Temp_Best),'delimiter','\t','newline','pc');
