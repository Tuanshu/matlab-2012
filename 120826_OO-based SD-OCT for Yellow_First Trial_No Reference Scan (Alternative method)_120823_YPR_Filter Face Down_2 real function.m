...
Sample_Path='D:\120824_YPR_Down\YPR\';
...
Position_Upper2Reference=(15.85+[-0.5:0.05:0.5])*1E-6;
Number_of_Loop=100;
Thickness=(4.4+[-0.2:0.05:0.2]).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6; In this file, it is the initial guess of thickness
Load_Previous_Result=0;
Ratio_Lower2Upper=1+[-0.2:0.05:0.2];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.6;
delta_n=0.4;
...
plot(Wavelength_micron(Wavelength_micron<1),Spectrum_Devided(Wavelength_micron<1),Wavelength_micron(Wavelength_micron<1),Spectrum_Devided_2(Wavelength_micron<1));
        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference)
                    ...
                    B = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now,Position_Upper2Reference_Now) abs(exp(i*4*pi.*Frequency_Considered.*Position_Upper2Reference_Now/C).*Ratio_Upper2Reference_Now./r_BK7_Considered.*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(i*2.*Frequency_Considered.*n.*Thickness_Now/C)./(1-r1_r(n).*r2(n).*exp(i*2*Frequency_Considered.*n.*Thickness_Now/C)))));
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        if Current_Loop<10 && Current_Loop>1
                            n_Record(:,qqqq)=n;
                            qqqq=qqqq+1;
                        end
...
plot(Wavelength_micron_Considered,real(n_Best),Wavelength_micron_Considered,imag(n_Best),Wavelength_micron_Considered,n1_Considered);
plot(Wavelength_micron_Considered,A(n,Thickness_Best,Ratio_Lower2Upper_Best,Ratio_Upper2Reference_Best,Position_Upper2Reference_Best),Wavelength_micron_Considered,Aexp);
%plot(Wavelength_micron_Considered,Spectrum_Sample_Upper_Temp_Best,Wavelength_micron_Considered,Spectrum_Sample_Lower_Temp_Best,Wavelength_micron_Considered,Spectrum_Sample_Considered);
plot(Wavelength_micron_Considered,T_Best,Wavelength_micron_Considered,T_Considered);
