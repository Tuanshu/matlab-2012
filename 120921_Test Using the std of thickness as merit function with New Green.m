...
Ratio_Upper2Reference=1.2+[-0.1:0.01:0.1];
Position_Upper2Reference=(15.85)*1E-6;
Number_of_Loop=75;
Thickness=(2.75).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6; In this file, it is the initial guess of thickness
Load_Previous_Result=0;
Ratio_Lower2Upper=1.3+[-0.1:0.01:0.1];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.2;
...
%end
Filter_inter=importdata('TEST200.txt');  
...
Glass_inter=importdata('TEST50.txt');  
%for j=0:(Averaging-1)
...
Spectrum_Sample_Wavelength=Filter_inter(:,2);
Spectrum_Reference_Wavelength=Glass_inter(:,2);
...
Separation_Position=21.25;   %(micron)
            ...
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        Merit=std(Thickness_Now)/mean(Thickness_Now);
