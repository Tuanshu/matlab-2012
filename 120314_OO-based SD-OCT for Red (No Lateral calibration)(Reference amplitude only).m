...
Ratio2=1;
Number_of_Loop=5;
Thickness=2.52.*1E-6; %[-0.2:0.01:0.2]*1E-6+
Load_Previous_Result=0;
Position_Error=27.45*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.2;
...
Spectrum_Devided=Spectrum_Sample./abs(Spectrum_Reference);

plot(Position_micron,Signal_Sample,Position_micron,Signal_Reference);

plot(Wavelength_micron(Wavelength_micron<0.65),abs(Spectrum_Reference(Wavelength_micron<0.65,:)));
...
if index_calculation==1   
    ...
    n_final(1:length(Frequency),1:length(Position_Error))=1.5;
    k_final(1:length(Frequency),1:length(Position_Error))=0;
    T_check(1:length(Frequency),1:length(Position_Error))=0;  
    Spectrum_Check(1:length(Frequency),1:length(Position_Error))=0;
 
    for q=1:length(Position_Error)
        Position_Error_Now=Position_Error(q);
        ...
        for p=1:Number_of_Loop
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                Spectrum_Upper_Temp=((r1).*(d0.^2)./r_BK7(index_now));                       %神說是cosine
                Spectrum_Lower_Temp=((t1.*t1_r.*r2.*(d.^2).*(d0.^2))./r_BK7(index_now));                         %神說是cosine
                Spectrum_Temp=Spectrum_Upper_Temp+Spectrum_Lower_Temp;
                
                Merit=(((Spectrum_Temp)-(Spectrum_Devided(index_now))).^2); %(abs(abs(Spectrum_Temp)-abs(Spectrum(index_now))).^2)+
...
for q=1:size(n_final,2)
    if index_calculation == 1
    r1_check=(n1-(n_final(:,q)+i*k_final(:,q)))./(n_final(:,q)+n1+i*k_final(:,q));
    r1_r_check=((n_final(:,q)+i*k_final(:,q))-n1)./(n_final(:,q)+n1+i*k_final(:,q));    
    t1_check=2*(n1)./(n_final(:,q)+n1+i*k_final(:,q));
    t1_r_check=2*(n_final(:,q)+i*k_final(:,q))./(n_final(:,q)+n1+i*k_final(:,q));
    t2_check=2*(n_final(:,q)+i*k_final(:,q))./(n_final(:,q)+n2+i*k_final(:,q));
    t2_r_check=2*(n2)./(n_final(:,q)+n2+i*k_final(:,q));
    r2_check=((n_final(:,q)+i*k_final(:,q))-n2)./((n_final(:,q)+i*k_final(:,q))+n2);   
    d_n_check=exp(i*2*pi.*Frequency/C.*(n_final(:,q)).*Thickness_Now);   %注意! -1*n!
    d_check=exp(i*2*pi.*Frequency/C.*(n_final(:,q)+i*k_final(:,q)).*Thickness_Now);   %注意! -1*n!
    d0_check=exp(i*2*pi.*Frequency/C.*(n_final(:,q)+i*k_final(:,q)).*Thickness_Now);   %注意! -1*n!
    %d0_check=exp(i*2*pi.*Frequency/C.*(Distance_fit(q)-Thickness));   %注意! -1*n!

    Spectrum_Upper_Temp_Check(:,q)=((r1_check)./r_BK7);                       %神說是cosine
    Spectrum_Lower_Temp_Check(:,q)=((t1_check.*t1_r_check.*r2_check.*(d_check.^2))./r_BK7); 
    T_abs_check(:,q)=abs(t1_check.*t1_r_check.*t2_check.*t2_r_check.*(d_check.^2));
    end
end
