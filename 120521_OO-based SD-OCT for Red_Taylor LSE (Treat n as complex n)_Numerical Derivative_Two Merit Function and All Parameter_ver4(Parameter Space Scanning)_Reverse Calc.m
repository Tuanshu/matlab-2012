...
Ratio_Reference2Sample=1.23;%+[-0.2:0.1:0.2];
Ratio_Upper2Lower=1.38;%+[-0.2:0.1:0.2];
Number_of_Loop=100;
Thickness=(2.54+[-0.1:0.02:0.1]).*1E-6;% ([-0.1:0.1:0.1]+2.55).*1E-6;
Load_Previous_Result=0;
Position_Upper_Interface=(10.7).*1E-6;  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.4;
...
cd('D:\120418\');
Spectrum_Upper_Temp_Best=importdata('Spectrum_Upper_Temp_Best_real.txt')+i*importdata('Spectrum_Upper_Temp_Best_imag.txt');
Spectrum_Lower_Temp_Best=importdata('Spectrum_Lower_Temp_Best_real.txt')+i*importdata('Spectrum_Lower_Temp_Best_imag.txt');
        ...
        for q=1:length(Position_Upper_Interface)
            for w=1:length(Ratio_Reference2Sample)
                for m=1:length(Ratio_Upper2Lower)
                    ...
                    Spectrum_Upper_Temp = @ (n,Thickness_Now,Position_Upper_Interface_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now) (exp(i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now))./r_BK7_Considered./Ratio_Reference2Sample_Now) .* (Ratio_Upper2Lower_Now.*(n1_Considered-n)./(n1_Considered+n));
                    Spectrum_Lower_Temp = @ (n,Thickness_Now,Position_Upper_Interface_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now) (exp(i*4*pi.*Frequency_Considered/C.*(Position_Upper_Interface_Now))./r_BK7_Considered./Ratio_Reference2Sample_Now) .* ((exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now) ./ (1-((n-n1_Considered)./(n+n1_Considered)) .* ((n-n2_Considered)./(n+n2_Considered)) .* exp(i*4*pi.*Frequency_Considered/C.*n*Thickness_Now))) .* ((2*n1_Considered)./(n1_Considered+n)) .* ((2*n)./(n1_Considered+n)) .* ((n-n2_Considered)./(n+n2_Considered)));
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        Merit=sum((Spectrum_Upper_Temp_Best-Spectrum_Upper_Temp(n,Thickness_Now,Position_Upper_Interface_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now)).^2)+sum((Spectrum_Lower_Temp_Best-Spectrum_Lower_Temp(n,Thickness_Now,Position_Upper_Interface_Now,Ratio_Reference2Sample_Now,Ratio_Upper2Lower_Now)).^2);
                        if Merit < Merit_Best
...
%plot(Wavelength_micron_Considered,Spectrum_Sample_Upper_Temp_Best,Wavelength_micron_Considered,Spectrum_Sample_Lower_Temp_Best);
...
%cd('D:\120418\');
%dlmwrite('Spectrum_Upper_Temp_Best.txt',Spectrum_Upper_Temp_Best,'delimiter','\t','newline','pc');
%dlmwrite('Spectrum_Lower_Temp_Best.txt',Spectrum_Lower_Temp_Best,'delimiter','\t','newline','pc');