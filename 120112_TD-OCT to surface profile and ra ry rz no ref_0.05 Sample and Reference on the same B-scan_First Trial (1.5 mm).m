...
data_Bscan=importdata('D:\120120\1.5 mm 2 interfaces.txt');
...
ref_Bscan_env=data_Bscan_env; 


imagesc(10*log10(data_Bscan_env));

data_Bscan_env(1:750,:)=0;
data_Bscan_env(1150:end,:)=0;
...
if ref_sub==1 
    for j=1:size(data_Bscan_env,2) 
        ref_Bscan_env(max((index_max_another(j)-2*window_ref),1):min((index_max_another(j)+window_ref),size(ref_Bscan_env,1)),j)=0;
    end
