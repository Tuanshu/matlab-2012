...
Ratio_Upper2Reference=0.8+[-0.1:0.02:0.1];
...
Ratio_Lower2Upper=0.8+[-0.1:0.02:0.1];  %([-1:0.02:1]+10.82)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.55;
delta_n=0.2;
...
for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)-1
    index_now=w+1;
    if (Cexp(index_now)-Cexp(index_now-1))>2*pi*0.9
        Cexp(index_now:end)=Cexp(index_now:end)-2*pi;
    elseif (Cexp(index_now)-Cexp(index_now-1))<-2*pi*0.9
        Cexp(index_now:end)=Cexp(index_now:end)+2*pi;
    end
end
...
B = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now) abs(Ratio_Upper2Reference_Now./r_BK7_Considered.*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(i*2.*Frequency_Considered.*n.*Thickness_Now/c))));
C = @ (n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now) angle(1./r1(n).*(Ratio_Lower2Upper_Now.*(t1(n).*t1_r(n).*r2(n).*exp(i*2.*Frequency_Considered.*n.*Thickness_Now/c))));
            ...
            for w=1:length(Ratio_Upper2Reference)
                for m=1:length(Position_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        if Current_Loop<80 && Current_Loop>70
                            n_Record(:,qqqq)=n;
                            qqqq=qqqq+1;
                        end
