...
%Position_Error_Temp=[0.017:0.001:0.027]*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;0.022

Position_Error_Temp=0.022E-6;
...
Data=importdata(sprintf('TEST%i.txt',Frame_index));
...
Position_Error=Position_Error_Temp;
...
% For Spectroscopy    
    ...
    for j=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
        ...
        if (index_now == Frequency_Center_Index) || (index_now == (Frequency_Center_Index -1))
            ...
            Merit(1:index_k_pre)=Merit(1:index_k_pre)*2;
            [value index]=min(Merit);
        end
        ...
        data_k=importdata('k_final_fit.txt');
        k=interp1(data_k(:,1),data_k(:,2),Frequency);
        k(isnan(k_final))=0;
    end
