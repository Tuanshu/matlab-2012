        ...
        for q=1:length(Ratio_Lower2Upper)
            for w=1:length(Ratio_Upper2Reference)
                    ...
                    while (Current_Loop<Number_of_Loop)
                        ...
                        delta_A=Aexp-repmat(A(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now),1,size(Aexp,2));
                        delta_B=Bexp-repmat(B(n,Thickness_Now,Ratio_Lower2Upper_Now,Ratio_Upper2Reference_Now),1,size(Bexp,2));
                        delta_C=Cexp-repmat(C_Temp,1,size(Cexp,2));
