...
Position_Error=(21.7)*1E-6;%[0.019:0.001:0.024]*1E-6;%[0.02:0.0003:0.023]*1E-6;
%%
n_should=1.7;
delta_n=0.1;
...
elseif strcmp(Scanning,'Thickness')
    Parameter=Thickness;
elseif strcmp(Scanning,'Position_Error')
    Parameter=Position_Error;
    ...
    elseif strcmp(Scanning,'Ratio')
        ...
        for p=1:Number_of_Loop
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                Spectrum_Lower_Temp=Ratio_Now*((t1.*t1_r.*r2.*(d.^2))./r_BK7(index_now))./(1-r1_r.*(d.^2).*r2).*(d0.^2);                         %神說是cosine                
                ...
                if Wavelength_micron(index_now) < 0.59
                    %Merit(T_abs>0.1)=max(Merit);
                elseif Wavelength_micron(index_now) > 0.62
                    %Merit(T_abs<0.5)=max(Merit);
                end
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                Spectrum_Upper(index_now)=Spectrum_Upper_Temp(index);
                Spectrum_Lower(index_now)=Spectrum_Lower_Temp(index);
            end
            

        end
        Spectrum_Upper_Record(:,q)=Spectrum_Upper;
        Spectrum_Lower_Record(:,q)=Spectrum_Lower;
        n_final(:,q)=n;
        k_final(:,q)=k;    
    end
...
plot(Wavelength_micron(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index),T_abs_check_smooth(Frequency_Considered_Min_Index:Frequency_Considered_Max_Index,1));
