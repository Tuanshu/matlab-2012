    ...
    elseif strcmp(Scanning,'Position_Error')
        ...
        for p=1:Number_of_Loop
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                t2_r=2*(n2(index_now))./(n_temp+n2(index_now)+i*k(index_now));
            ...
            for w=1:(Frequency_Considered_Max_Index-Frequency_Considered_Min_Index+1)
                ...
                t2_r=2*(n2(index_now))./(n(index_now)+n2(index_now)+i*k_temp);
...
for j=1:length(Parameter)
    if index_calculation == 1        
    t_sub=2.*n2(index_now)./(n2(index_now)+n1);
    ...
    t2_check=2*(n_final(:,j)+i*k_final(:,j))./(n_final(:,j)+n2(index_now)+i*k_final(:,j));
    t2_r_check=2*(n2(index_now))./(n_final(:,j)+n2(index_now)+i*k_final(:,j));
    r2_check=((n_final(:,j)+i*k_final(:,j))-n2(index_now))./((n_final(:,j)+i*k_final(:,j))+n2(index_now));   
