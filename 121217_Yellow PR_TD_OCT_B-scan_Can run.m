...
TD_OCT_point_Spacing=0.0075;      %micron 因為我取的點數是原先的四倍, 故好像應該是169.8/4800/4=0.00884375
...
Signal_Sam_Filtered=ifft(Spectrum_Sam,[],1);
%Signal_Ref_Filtered=ifft(Spectrum_Ref,[],1);

plot(Frequency,real(Spectrum_Sam(:,:))); 
plot(Position_micron,Signal_Sam_Filtered(:,:));%,Position_micron,Signal_Ref_Filtered(:,:));
...
Spectrum_Sample_1=fft(Signal_Sample_1,[],1);
%Spectrum_Sample_1=2*Spectrum_Sample_1(1:N_f,:);                

Spectrum_Sample_2=fft(Signal_Sample_2,[],1);
...
plot(Wavelength_micron(Wavelength_micron<1),real(Spectrum_Sample_1(Wavelength_micron<1,:)));           %若是頻譜解析度太差, 是不是也會因為Signal 
